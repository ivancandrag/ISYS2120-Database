...
# show trips on a particular date (user input)

def search_trips_date(attribute, filterval):
    # Get the database connection and set up the cursor
    conn = database_connect()
    if(conn is None):
        # If a connection cannot be established, send an Null object
...
# add a new trip (user input)

def add_trip_insert(cardid, traveldate, entrystationid, exitstationid, tripstarttime):
    """
    ...
    sql = """
        INSERT into opaltravel.Trips(cardid, traveldate, entrystationid, exitstationid, tripstarttime)
        VALUES (%s,%s,%s,%s,%s);
        """
    print_sql_string(sql, (cardid, traveldate, entrystationid, exitstationid, tripstarttime))
    try:
        # Try executing the SQL and get from the database
        sql = """
        INSERT into opaltravel.Trips(cardid, traveldate, entrystationid, exitstationid, tripstarttime)
        ...
        cur.execute(sql,(cardid, traveldate, entrystationid, exitstationid, tripstarttime))
...
# update other fields for a particular trip

def update_single_trip(tripid, cardid, traveldate, entrystationid, exitstationid, tripstarttime):
    # Get the database connection and set up the cursor
    conn = database_connect()
    if(conn is None):
        # If a connection cannot be established, send an Null object
    ...
    try:
        ...
        if cardid is not None:
            setitems += "cardid = %s::bigint\n"
            attcounter += 1
        if traveldate is not None:
            if attcounter != 0:
                setitems += ","
            setitems += "traveldate = %s::date\n"
            attcounter += 1
        if entrystationid is not None:
            if attcounter != 0:
                setitems += ","
            setitems += "entrystationid = %s::bigint\n"
            attcounter += 1
        if exitstationid is not None:
            if attcounter != 0:
                setitems += ","
            setitems += "exitstationid = %s::bigint\n"
            attcounter += 1
        if tripstarttime is not None:
            if attcounter != 0:
                setitems += ","
            setitems += "tripstarttime = %s::time\n"
            attcounter += 1

        # Retrieve all the information we need from the query
        sql = f"""UPDATE opaltravel.trips
                    SET {setitems}
                    WHERE tripid = {tripid};"""
        print_sql_string(sql,(cardid, traveldate, entrystationid, exitstationid, tripstarttime))
        val = dictfetchone(cur,sql,(cardid, traveldate, entrystationid, exitstationid, tripstarttime))
        conn.commit()
...
#produce a report showing the various stationtypes (by stationtypeid) 
#alongside with the number of stations that belong to each of those stationtypes.
...
def list_trip_stats():
    ...
    try:
        # Set-up our SQL query
        sql = """SELECT entrystationid, COUNT(*) as count
                FROM opaltravel.trips 
                    GROUP BY entrystationid
                    ORDER BY entrystationid ASC ;"""
...
# Get all rows in users where a particular attribute matches a value
def list_trips_equifilter(attributename, filterval):
    # Get the database connection and set up the cursor
    conn = database_connect()
    if(conn is None):
        # If a connection cannot be established, send an Null object
